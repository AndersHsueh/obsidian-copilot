...
export class DBOperations {
  ...
  async initializeDB(embeddingInstance: Embeddings | undefined): Promise<Orama<any> | undefined> {
    try {
      ...
      new Notice("Failed to initialize Obsidian-Mate database. Some features may be limited.");
      return undefined;
    }
  }
  ...
  async clearIndex(embeddingInstance: Embeddings | undefined): Promise<void> {
    try {
      ...
      new Notice("Local Obsidian-Mate index cleared successfully.");
      logInfo("Local Obsidian-Mate index cleared successfully, new instance created.");
    } catch (err) {
      logError("Error clearing the local Obsidian-Mate index:", err);
      new Notice("An error occurred while clearing the local Obsidian-Mate index.");
      throw err;
    }
  }
  ...
  public async removeDocs(filePath: string) {
    ...
    try {
      ...
      if (searchResult.hits.length > 0) {
        ...
        if (getSettings().debug) {
          logInfo(`Deleted document from local Obsidian-Mate index: ${filePath}`);
        }
      }
      this.markUnsavedChanges();
    } catch (err) {
      logError("Error deleting document from local index:", err);
    }
  }
  ...
  async checkAndHandleEmbeddingModelChange(embeddingInstance: Embeddings): Promise<boolean> {
    ...
    if (prevEmbeddingModel) {
      ...
      if (!areEmbeddingModelsSame(prevEmbeddingModel, currEmbeddingModel)) {
        ...
        new Notice("New embedding model detected. Rebuilding Obsidian-Mate index from scratch.");
        logInfo(
          `Detected change in embedding model from "${prevEmbeddingModel}" to "${currEmbeddingModel}". Rebuilding Obsidian-Mate index from scratch.`
        );
  ...
  public async garbageCollect(): Promise<number> {
    ...
    try {
      ...
      logInfo(
        "Obsidian-Mate index: Docs to remove during garbage collection:",
      ...
      logError("Error garbage collecting the Obsidian-Mate index:", err);
      throw new CustomError("Failed to garbage collect the Obsidian-Mate index.");
    }
  }
